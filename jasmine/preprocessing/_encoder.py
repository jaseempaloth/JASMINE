...
def fit_fn(X: jnp.ndarray, categories: Union[str, List[jnp.ndarray]] = "auto") -> EncoderParams:
    ...
    if categories == "auto":
        ...
        if X.dtype.kind in ["U", "S", "O"]:
            learned_categories = [np.unique(X[:, i]) for i in range(X.shape[1])]
        else:
            learned_categories = [jnp.unique(X[:, i]) for i in range(X.shape[1])]
    else:
        learned_categories = cast(List[jnp.ndarray], categories)
            ...
            for i, cats in enumerate(self.params["categories"]):
                ...
                if feature_column.dtype.kind in ["U", "S", "O"]:
                    ...
                    if np.any(is_unknown):
                        ...
                        raise ValueError(
                            f"Found unknown category {unknown_value} in feature {i} during"
                        )
                else:
                    ...
                    if jnp.any(is_unknown):
                        ...
                        raise ValueError(
                            f"Found unknown category {unknown_value} in feature {i} "
                            "during transform."
                        )
